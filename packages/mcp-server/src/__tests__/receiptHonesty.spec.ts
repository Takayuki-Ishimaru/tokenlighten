...
// ---------------------------------------------------------------------------
// post-ready discovery trim: no longer flag-gated (the v0.14 flag-inventory
// surgery deleted the W5 TL_POST_READY_TRIM/_N counter from
// guardExecutionDiscovery) — a plain full read stays untrimmed
// ---------------------------------------------------------------------------

describe("post-ready discovery trim: a plain full read is never trimmed", () => {
  it("stays a normal full serve across repeated reads, with no post-ready-trim wording on the wire", async () => {
    const { ws, srv } = await liveServer("post-ready-full-read");
...
describe("[R5-10] F-1b — a serve the cap dropped stays discovery-eligible", () => {
  it("a mode=full downgraded past the per-task cap ships no body, and the slice retry SERVES", async () => {
    ...
    const total = 12;
    // FX-G13 G3 (2026-09-04) exempts any file at or under TINY_BYTES (8192)
    // from the per-task full-read cap outright — the cap this case pins can
    // only fire on a file it does NOT consider small, so each fixture file
    // must clear that byte floor (800 pad lines lands every file at roughly
    // 9.5-10.3 KB, comfortably over 8192 for every tag width used below).
...
describe("overlap serving: a read overlapping an earlier one serves its full body", () => {
  // No longer flag-gated: the v0.14 flag-inventory surgery deleted the W7
  // TL_OVERLAP_TRIM slice-partition synthesis block from server.ts (the
  // shared segments/code_unchanged projector it built on, and delta-context,
  // are untouched). A read overlapping an earlier one now always serves its
  // full body — never a trimmed residual with a 'prior' projection.
  const overlapFixture = (): string => Array.from(
    { length: 12 },
    (_, index) => `export const LINE_${String(index + 1).padStart(2, "0")} = ${index + 1};`,
  ).join("\n") + "\n";

  it("keeps the pre-W7 full-body behavior — no residual trim, no 'prior' projection", async () => {
    const { ws, srv } = await liveServer("overlap-full-body");
...
describe("[R5-10] the served-range ledger books what the wire carried", () => {
  ...
  it("an unattributable body fails CLOSED — it corroborates no window, so nothing is booked (FX-O1, ruling (t))", () => {
    // WAS: "fails OPEN — a projector gap must not re-serve held bytes", which
    // kept every span AND (via `_settleSessionServeBookings`'s fail-open arm)
    // promoted every pending path to `shipped` write authority. Round-17
    // finding 2 measured the cost: a staged residue left behind by a call that
    // threw before the funnel was promoted by the NEXT unattributable response
    // — and `mode=symbol`'s assembled scope view is an ordinary production
    // response that projects `unattributed: true`. Ruling (t): a response that
    // cannot attribute its bytes proves nothing, so it books nothing. The
    // re-serve this arm used to avoid is paid by no production shape: the two
    // pathless serve payloads now publish `serveAttributionPath`
    // (`protocol/envelope.ts`), so the flag is unreachable for a shipped body.
    ...
    expect(servedRangeReceipt(WS, "src/openfail.ts", SHA, 1, 40, 200)).toBeUndefined();
  });

  it("an unattributable body promotes NO path — the residue of a throwing call cannot be laundered (FX-O1)", () => {
    // The concrete shape round-17 finding 2 demonstrated in-process: a call
    // throws after staging a span for a file, leaving the staging behind; the
    // next call is an unattributable response for a DIFFERENT file. Under the
    // old fail-open arm the leaked path became `"shipped"` and `edit_file`
    // applied to a file no byte of which had been sent.
    recordServedRange(WS, "src/leaked-residue.ts", SHA, 1, 20, 20, { mode: "full", range: "1-20", call: 1 });
    settleServedRanges(WS, { unattributed: true, windows: [] });
    const session = getSession(WS);
    expect(session.admissibleEditPaths, "no write authority from an unattributable response")
      .not.toContain("src/leaked-residue.ts");
    expect(session.editPathResidency.get("src/leaked-residue.ts")).toBeUndefined();
  });
