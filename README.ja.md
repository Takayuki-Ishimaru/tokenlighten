...
**パブリックベータ。** TokenLighten v0.14.3は、エディタ拡張機能もNode.jsの個別インストールも不要な導入方法を追加するアップデートです。OSごとに1つのアーカイブをダウンロードし、1つのコマンドを実行するだけで導入できます。GitHub Copilotとの連携、タスクの継続、日本語の依頼、Windows対応も改善します。MCPツールは引き続き3つで、サーバーはデフォルトで読み取り専用です。フィードバックに応じて、インターフェースや対応ワークフローが変更される場合があります。

v0.14.3の主な変更点は次のとおりです。

- `tl-setup <ワークスペース>`で、Node.jsランタイムを同梱したアーカイブからTokenLightenをインストールし、検出されたAIエージェントホスト（Claude Code、Codex、Gemini CLI、Copilot CLI、VS CodeのGitHub Copilot Chat）を登録して、ワークスペースをセットアップします。再実行するとその場でアップグレードでき、`--use`でロールバック、`--uninstall`で書き込んだエントリやガイドブロックごとアンインストールできます。
- VS Code拡張機能の**このワークスペースをセットアップ**は同じマシン単位のインストールを行い、ワークスペースの設定ファイルに加えて2つ目のサーバー定義を登録しなくなりました。マシンからアンインストールするコマンドも追加しました。
- `tl doctor`がマシンインストール・拡張機能・`PATH`上の`tl`の整合性を報告し、`tl clients snippet`が直接書き込めないホスト向けの貼り付け用エントリを出力します。
- タスクの継続時に、実行済みの検索を繰り返したり、保留中のファイル作成を失ったり、返却済みの本文を再送したりしなくなりました。複数のファイルや変更を挙げた依頼は、英語でも日本語でもより正確に読み取ります。
- 複数のことを一度に尋ねる読み取り専用の質問で、最初の応答が各論点の根拠を含むことが多くなりました。英語のコードベースに対する日本語の質問は、そこから導いた英語の検索語で再試行します（どちらも既定で有効。`TL_CONCERN_RECOVERY=0`、`TL_JA_QUERY_BRIDGE=0`で無効化できます）。
- 読み取り専用で始めたタスクは、続きの呼び出しが`task.profile`を省略しても読み取り専用のままです。モデルが崩したタスクハンドルは、`read_file`と`search_files`では呼び出し元自身の継続中タスクとして続行します（`TL_TASK_HANDLE_RECOVERY=0`で従来の厳格な拒否に戻せます）。
- VS CodeのGitHub Copilot向けに、1回の応答で返す関連情報を充実させ、ツール定義とワークスペースの指示を整理しました。Copilotのみで使うワークスペースでは、セットアップ時にコンパクトなガイドを選べます。
- 複数のファイルを行範囲つきで指定した`read_file`が、応答の短縮時に行を取りこぼさなくなりました。複数の独立した質問も、すべての質問をカバーするまで完了扱いにしません。
- すべてのファイル本文が1つのデコード方針を通るようになり、デコードできないファイルやNULバイトの多いファイルは配信せずにその旨を開示します。
- Windowsで、アーカイブの読み取り、クライアントの登録、同梱ランタイムの扱いが動作するようになりました。エクスプローラーで展開したフォルダーからでも導入できます。

公開リリースには以下が含まれます。

- CLI、MCPサーバー、Node.jsランタイムを同梱した4種類のインストール用アーカイブ（win-x64、darwin-arm64、darwin-x64、linux-x64）
- 開発者向けのソースコードと公開パッケージテスト（TokenLighten CLIとMCPサーバー）
- 単体で動作するVSIX形式のVS Code拡張機能

**互換性の注意：** 3つのツールと現行のリクエスト項目は引き続き利用できます。アップグレード後は`tl workspace setup`または拡張機能の**このワークスペースをセットアップ**を再実行し、管理ガイドとCopilot向けの設定を更新してください。v0.14.3のいずれかの入口を初めて実行すると、従来の`~/.tokenlighten/bin/tl`ランチャーは転送用ファイルへ移行され、管理対象のホスト登録は新しい識別情報へ付け替えられます。v0.12／v0.13の旧リクエスト項目は引き続きデフォルトで受け付けません（サーバー側の`TL_LEGACY_INPUT=accept`は一時的な移行手段として利用できます）。詳細と既知の制限は、[v0.14.3リリースノート](release-docs/github-release-v0.14.3.md)を参照してください。
...
これらは参考値であり、削減を保証するものではありません。掲載した数値はv0.14.0のものであり、v0.14.3やGitHub Copilotなど特定のホストでの性能を示すものではありません。結果はリポジトリ、タスク、クライアント、モデルの動作、料金によって異なります。ご自身のワークスペースでは、CLIやVS Codeの使用量表示でローカルの計測値と推定値を確認できます。これらの表示はプロバイダーの請求記録ではありません。
...
## アーカイブをインストールする（拡張機能もビルドも不要）

[最新のGitHub Release](https://github.com/Takayuki-Ishimaru/tokenlighten/releases)からご利用のOS向けアーカイブをダウンロードし、公開されている`SHA256SUMS`で検証してから展開し、次を実行します。
...
各アーカイブには専用のNode.jsが同梱されているため、別途ランタイムを導入する必要も、エディタ拡張機能も不要です。この操作により、検出されたすべてのAIエージェントホスト（Claude Code、Codex、Gemini CLI、Copilot CLI、およびVS Code内のGitHub Copilot Chat）向けにTokenLightenをマシン単位のMCPサーバーとして登録し、指定したワークスペースをセットアップします。検証・アップグレード・ロールバック・アンインストールの手順は[はじめに](release-docs/getting-started.md)を参照してください。

VS Code拡張機能を使いたい場合は、次の節を参照してください。

## VS Code拡張機能をインストールする（ビルド不要）

v0.14.3のGitHub Releaseから[tokenlighten-vscode-extension-0.14.3.vsix](https://github.com/Takayuki-Ishimaru/tokenlighten/releases/download/v0.14.3/tokenlighten-vscode-extension-0.14.3.vsix)をダウンロードしてください。Node.jsの導入やソースからのビルドは不要です。このリリースにはOS固有のネイティブバイナリが含まれないため、Windows、macOS、Linuxで同じVSIXを使用します。
...
信頼済みのプロジェクトフォルダを開き、TokenLightenビューから**このワークスペースをセットアップ**を選択します。VSIXにはCLI、MCPサーバー、パーサー、必要なアセットが含まれるため、別途グローバルインストールする必要はありません。この操作は、上記のアーカイブと同じマシン単位のインストール（`tl install --from-extension`）を実行し、現在のワークスペースをセットアップしますが、このマシン上のClaude CodeやCodexなど他のホストは登録しません（`--clients none`）。これらを登録するには、`tl clients activate`を実行するか、アーカイブの`tl-setup`を使用してください。
...
`npm link`はこのチェックアウトだけで有効な`tl`コマンドです。他のホストからも見つかるマシンインストールにするには、続けて`tl install --dev`を実行してください。アーカイブインストールと同じIDになります。
