...
**パブリックベータ。** TokenLighten v0.14.1は、コーディングエージェントが「必要な情報を読み終えた」と判断する条件と、途中で打ち切られた読み込み・検索の継続方法を改善する信頼性アップデートです。MCPツールは引き続き3つで、サーバーはデフォルトで読み取り専用です。フィードバックに応じて、インターフェースや対応ワークフローが変更される場合があります。

v0.14.1の主な変更点は次のとおりです。

- 要求に含まれる各論点を、実際に返した内容が覆うまで、回答・編集の準備ができたとは報告しません。
- 応答サイズの上限を超えた読み込みは、要求した行をすべて届けるまで`cursor`で継続します。
- 範囲を制限した検索は検索のまま継続し、ファイル全体の読み込みへ膨らみません。すでにコンテキストにある内容は、明示的に要求しない限り再送しません。
- 既存ファイルの変更と新規ファイルの作成を一括で実行でき、`target:"all"`で一致する箇所をすべて正しく置換できます。
- ソースコード・テキスト・設定ファイルだけのワークスペース向けに、より小さい`code`ツールサーフェスを追加しました。
...
**互換性の注意：** リクエスト形式は`cursor`項目の追加を除いて変わりません。v0.12／v0.13の旧リクエスト項目は引き続きデフォルトで受け付けません（移行期間中は、サーバー側で`TL_LEGACY_INPUT=accept`を設定すると旧形式を一時的に利用できます）。`content:"full"`は、すでにコンテキストにある内容の再送を強制しなくなりました。再送が必要な場合は`task.force_serve:true`を指定してください。互換性の詳細と既知の制限は、[v0.14.1リリースノート](release-docs/github-release-v0.14.1.md)を参照してください。
...
これらは参考値であり、削減を保証するものではありません。この比較はv0.14.0で行ったもので、v0.14.1では再計測していません。結果はリポジトリ、タスク、クライアント、モデルの動作、料金によって異なります。ご自身のワークスペースでは、CLIやVS Codeの使用量表示でローカルの計測値と推定値を確認できます。これらの表示はプロバイダーの請求記録ではありません。
...
v0.14.1のGitHub Releaseから[tokenlighten-vscode-extension-0.14.1.vsix](https://github.com/Takayuki-Ishimaru/tokenlighten/releases/download/v0.14.1/tokenlighten-vscode-extension-0.14.1.vsix)をダウンロードしてください。Node.jsの導入やソースからのビルドは不要です。このリリースにはOS固有のネイティブバイナリが含まれないため、Windows、macOS、Linuxで同じVSIXを使用します。
